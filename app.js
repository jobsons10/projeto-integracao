...
function handleCredentialResponse(response) {
  ...
  if (data) {
    ...
    if (data.email.includes("@grupootg") || data.email.includes("@compayz")) {
      // VERIFICAR SE O O EMAIL LOGADO É CORPRATIVO
      fetch(`https://api-otg.herokuapp.com/user/${data.email}`)
...
const gameAnimation = () => {
  ...
  ball.onclick = async function () {
    fetch("https://api-otg.herokuapp.com/award")
      .then((response) => response.json())
      .then((data) => {
        ...
        console.log(awardAmountUpdated);
        ball.classList.add(`${award.type}`);
        if (awardAmountUpdated === 0) {
          fetch(`https://api-otg.herokuapp.com/award/${award.name}`, {
            method: "DELETE",
          ...
          fetch(`https://api-otg.herokuapp.com/award/${award.name}`, {
            ...
            body: JSON.stringify({
              amount: awardAmountUpdated,
            }),
          }).catch((err) => console.log(err));
        }
        lottie.classList.remove("hide-me");
      //   setTimeout(() => {
